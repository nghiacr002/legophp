...
class Debug
{
    ...
    public static $app_start_time;
    public static $app_end_time;

    public static function executeTime()
    {
        self::$app_end_time = microtime(true);
        $time_exec = self::$app_end_time - self::$app_start_time;
        return round($time_exec, 6);
    }
        ...
        if ($init)
        {
            if (!self::$registered)
                self::$registered = array(
                    ...
                    , 'shutdown' => register_shutdown_function('\APP\Engine\Debug::shutdown')
                );
            self::$registered['shutdown'] = true;
            error_reporting(E_ALL);
            set_error_handler('\APP\Engine\Debug::handler', E_ALL);
            set_exception_handler('\APP\Engine\Debug::exception');
    ...
    static public function handler($type, $message, $file, $line, $scope, $stack = null)
    {
        ...
        $content = array();
        $content[] = sprintf('[%s]: %s in <b>%s</b> [%s]'  // print error
        ...
        if ($type & self::$reporting) // print context
            $context = array_merge($content, self::context($stack, $scope));
...
//\App\Engine\AppException
    ...
    static protected function context($stack, $scope)
    {
        ...
        foreach ($stack as $index => $call)
            $context[] = sprintf('[#%s] %s: <b>%s%s%s</b><br/>'
                    , $index
    ...
    static protected function args_export($args)
    {
        return implode(', ', array_map(
                        '\APP\Engine\Debug::simple_export', (array) $args
        ));
    }
